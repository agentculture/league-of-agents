...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope league-of-agents
...
scope:** the default personal scope is **private** (`--scope league-of-agents
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `league-of-agents`). Private records are served only to a recall in the same
